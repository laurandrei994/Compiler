/* vim:set ft=c: -*- C++ -*- */
#ifndef QUEX_INCLUDE_GUARD_quexProcessed__BUFFER__BUFFER_LOAD_I
#define QUEX_INCLUDE_GUARD_quexProcessed__BUFFER__BUFFER_LOAD_I
...
inline E_LoadResult
quexProcessed_Buffer_load_forward(quexProcessed_Buffer*  me,
 ...
 *                                                                            */
{
    ...
    if( me->_read_p != me->content_end(me)) {
        ...
        quexProcessed_Buffer_register_eos(me, me->input.lexatom_index_begin + me->content_size(me));
    ...
    if(    0 == move_distance 
        && ! quexProcessed_Buffer_callbacks_on_cannot_move_towards_begin(me, &move_distance) ) {
        return E_LoadResult_OVERFLOW; 
    }

    if( ! quexProcessed_Buffer_move_and_load(me, position_register, PositionRegisterN, 
...
inline bool
quexProcessed_Buffer_load_forward_to_contain(quexProcessed_Buffer*        me, 
 ...
 *           *disabled*. 
 *
 *        => In case of 'return false', call 'quexProcessed_Buffer_dysfunctional */
{
    quexProcessed_stream_position_t    lexatom_index_to_be_contained = LexatomIndexToBeContained;
    quexProcessed_stream_position_t    lexatom_index_end = (quexProcessed_stream_position_t)-1;
    ...
    move_distance = quexProcessed_Buffer_move_get_distance_forward_to_contain(me, 
                                             &lexatom_index_to_be_contained);

    /* Even if 'move_distance = 0' call the 'move and load' for consistency.  */
    (void)quexProcessed_Buffer_move_and_load(me, (quexProcessed_lexatom_t**)0, 0,
    ...
    if( LexatomIndexToBeContained < lexatom_index_end ) {
        ...
        load_request_n = quexProcessed_Buffer_move_towards_begin_undo(me, &bi, move_distance);

        loaded_n       = quexProcessed_LexatomLoader_load(me->filler, me->content_begin(me), 
        ...
        if( loaded_n != load_request_n ) {
            ...
            quexProcessed_Buffer_dysfunctional_set(me);
        }
        else {
...
inline E_LoadResult   
quexProcessed_Buffer_load_backward(quexProcessed_Buffer* me)
 ...
 *      to go farther back then the buffer's begin.                          */
{
    ...
    else if( ! quexProcessed_ByteLoader_seek_is_enabled(me->filler->byte_loader) ) {
        return E_LoadResult_NO_MORE_DATA; /* Stream cannot go backwards.     */
    }

    move_distance = quexProcessed_Buffer_move_get_max_distance_towards_end(me);
    ...
    loaded_n = quexProcessed_Buffer_move_and_load_backward(me, move_distance, &encoding_error_f, &load_request_n);
...
inline bool
quexProcessed_Buffer_load_backward_to_contain(quexProcessed_Buffer*        me, 
                                           quexProcessed_stream_position_t NewCharacterIndexBegin)
...
{
    /* quexProcessed_lexatom_t* end_p    = me->content_space_end(me); */
    ptrdiff_t loaded_n;
    ptrdiff_t load_request_n;
    ptrdiff_t move_distance;
    bool      encoding_error_f = false;
    ...
    loaded_n = quexProcessed_Buffer_move_and_load_backward(me, move_distance, 
                                                        &encoding_error_f, 
                                                        &load_request_n);
...
inline bool
quexProcessed_Buffer_move_and_load(quexProcessed_Buffer*  me, 
                                quexProcessed_lexatom_t** position_register,
...
{
    quexProcessed_stream_position_t   load_lexatom_index;
    ptrdiff_t                   free_space;
    bool                        end_of_stream_f  = false;

    quexProcessed_Buffer_move_towards_begin(me, move_distance,
    ...
    *loaded_n = quexProcessed_LexatomLoader_load(me->filler, me->content_end(me), 
...
inline ptrdiff_t
quexProcessed_Buffer_move_and_load_backward(quexProcessed_Buffer* me, 
 ...
 * RETURNS: Number of loaded lexatoms.                                        */
{
    ...
    (void)quexProcessed_Buffer_move_towards_end(me, MoveDistance);

    *load_request_n = QUEX_MIN(me->content_space_end(me) - me->content_begin(me), 
                               MoveDistance);

    return quexProcessed_LexatomLoader_load(me->filler, me->content_begin(me), *load_request_n,
...
#endif /* QUEX_INCLUDE_GUARD_quexProcessed__BUFFER__BUFFER_LOAD_I */
