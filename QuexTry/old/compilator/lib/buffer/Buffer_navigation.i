...
#ifndef QUEX_INCLUDE_GUARD_compilator__BUFFER__BUFFER_NAVIGATION_I
#define QUEX_INCLUDE_GUARD_compilator__BUFFER__BUFFER_NAVIGATION_I
...
inline bool  
compilator_Buffer_seek_forward(compilator_Buffer* me, const ptrdiff_t CharacterN)
 ...
 *          False -- else.                                                    */
{
    compilator_stream_position_t  CharacterIndexAtReadP =   me->input.lexatom_index_begin
                                                       + (me->_read_p - me->content_begin(me));
    compilator_stream_position_t  target = CharacterIndexAtReadP + CharacterN;

    if( ! CharacterN ) {
        return true;
    }
    else if( target < compilator_Buffer_input_lexatom_index_end(me) ) {
        ...
        if( ! compilator_Buffer_load_forward_to_contain(me, target) ) {
    ...
    return compilator_Buffer_finish_seek_based_on_read_p(me);
}

inline bool  
compilator_Buffer_seek_backward(compilator_Buffer* me, 
 ...
 *          False -- else.                                                   */
{
    ...
    if( ! CharacterN ) {
        ...
        offset                  = (ptrdiff_t)QUEX_MIN((compilator_stream_position_t)(ContentSpaceSize >> 1), target);
        new_lexatom_index_begin = target - offset;

        if( ! compilator_Buffer_load_backward_to_contain(me, new_lexatom_index_begin) ) {
...
inline compilator_stream_position_t  
compilator_Buffer_tell(compilator_Buffer* me)
/* RETURNS: lexatom index which corresponds to the position of the input
 *          pointer.                                                         */
{
    const compilator_stream_position_t Delta = me->_read_p - me->content_begin(me);
    return Delta + me->input.lexatom_index_begin;
}

inline bool    
compilator_Buffer_seek(compilator_Buffer*              me, 
 ...
 *          totally fail. Then, check 'compilator_Buffer_dysfunctional'.      */
{
    const compilator_stream_position_t lexatom_index_read_p = compilator_Buffer_tell(me);
    bool  verdict_f = false;

    if( LexatomIndex > lexatom_index_read_p ) {
        verdict_f = compilator_Buffer_seek_forward(me, (ptrdiff_t)(LexatomIndex - lexatom_index_read_p));
    }
    else if( LexatomIndex < lexatom_index_read_p ) {
        verdict_f = compilator_Buffer_seek_backward(me,(ptrdiff_t)(lexatom_index_read_p - LexatomIndex));
    }
    return verdict_f;
}

inline bool
compilator_Buffer_finish_seek_based_on_read_p(compilator_Buffer* me)
{
...
#endif                  /* QUEX_INCLUDE_GUARD_compilator__BUFFER__BUFFER_NAVIGATION_I */
