/* vim:set ft=c: -*- C++ -*- */
#ifndef QUEX_INCLUDE_GUARD_Calc_lexer__BUFFER__BUFFER_MOVE_I
#define QUEX_INCLUDE_GUARD_Calc_lexer__BUFFER__BUFFER_MOVE_I
...
#include "lexical_analysis/lib/buffer/asserts"
 ...
 ptrdiff_t
Calc_lexer_Buffer_move_towards_begin(Calc_lexer_Buffer*  me, 
 ...
 * RETURNS: Number of free lexatoms from 'content_end' the end of buffer.     */
{
    ...
    if( MoveDistance ) {
        ...
        move_size = Calc_lexer_Buffer_move_size_towards_begin(me, MoveDistance);

        if( MoveDistance && move_size ) {
            __QUEX_STD_memmove((void*)me->content_begin(me), 
                               (void*)&me->content_begin(me)[MoveDistance],
                               (size_t)move_size * sizeof(Calc_lexer_lexatom_t));
        }

        Calc_lexer_Buffer_pointers_add_offset(me, - MoveDistance, 
 ...
 ptrdiff_t
Calc_lexer_Buffer_move_towards_end(Calc_lexer_Buffer* me, 
                                   ptrdiff_t          MoveDistance)
...
{
    Calc_lexer_lexatom_t*  BeginP           = me->content_begin(me);
    ...
    if( MoveDistance ) {
        ...
        Calc_lexer_Buffer_move_backup_lexatom_index_of_lexeme_start(me, MoveDistance);
        ...
        Calc_lexer_Buffer_pointers_add_offset(me, MoveDistance, (Calc_lexer_lexatom_t**)0, 0);
    }
 ...
 ptrdiff_t
Calc_lexer_Buffer_move_towards_begin_undo(Calc_lexer_Buffer*           me,
 ...
 * That is, they indicate the situation to be restored.                       */
{
    ...
    Calc_lexer_BufferInvariance_restore(bi, me);
    ...
    if( move_size && MoveDistance ) {
        /* NOT NECESSARY:
         *
         * Calc_lexer_Buffer_callbacks_on_buffer_before_change(me, BeginP);
         *
 ...
 ptrdiff_t
Calc_lexer_Buffer_move_get_max_distance_towards_begin(Calc_lexer_Buffer*  me) 
 ...
 *        begin of the buffer.                                                */
{
    ...
    else if( Calc_lexer_Buffer_is_end_of_stream_inside(me) ) {
 ...
 ptrdiff_t        
Calc_lexer_Buffer_move_get_max_distance_towards_end(Calc_lexer_Buffer* me)
 ...
 *        begin of the buffer.                                                */
{
 ...
 ptrdiff_t
Calc_lexer_Buffer_move_size_towards_begin(Calc_lexer_Buffer* me, ptrdiff_t move_distance)
{
 ...
 ptrdiff_t
Calc_lexer_Buffer_move_get_distance_forward_to_contain(Calc_lexer_Buffer*         me,
                                                       Calc_lexer_stream_position_t* lexatom_index_to_be_contained)
{
    Calc_lexer_stream_position_t lexatom_index_begin = me->input.lexatom_index_begin;
    Calc_lexer_stream_position_t lexatom_index_end   = lexatom_index_begin + me->content_size(me);
    Calc_lexer_stream_position_t new_lexatom_index_begin;
    const ptrdiff_t           FallBackN = me->_fallback_n;
    __quex_assert(FallBackN >= 0);
    ...
    __quex_assert(*lexatom_index_to_be_contained != (Calc_lexer_stream_position_t)-1);
    ...
    else if( me->input.lexatom_index_end_of_stream != (Calc_lexer_stream_position_t)-1 ) {
 ...
 void
Calc_lexer_Buffer_move_backup_lexatom_index_of_lexeme_start(Calc_lexer_Buffer* me,
                                                            ptrdiff_t          MoveDistance)
 ...
 * can be targetted and 'normal' analysis can continue.                       */
{
    if(    me->_lexeme_start_p
        ...
        me->_backup_lexatom_index_of_lexeme_start_p =   Calc_lexer_Buffer_tell(me)
...
#endif /* QUEX_INCLUDE_GUARD_Calc_lexer__BUFFER__BUFFER_MOVE_I */
