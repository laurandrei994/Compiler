...
#ifndef  QUEX_INCLUDE_GUARD_Calc_lexer__BUFFER__LEXATOMS__LEXATOM_LOADER_NAVIGATION_I
#define  QUEX_INCLUDE_GUARD_Calc_lexer__BUFFER__LEXATOMS__LEXATOM_LOADER_NAVIGATION_I
 ...
 *    -- Calc_lexer_ByteLoader's seek sets the position in the low level input
 *       stream.
 * 
 * A LexatomLoader's seek has NO direct influence on the content or the pointers
 * of the Buffer object that it fills.
 *
 * The difference between the Calc_lexer_ByteLoader's seeking (stream seeking) 
 * and the LexatomLoader's seeking is that the Calc_lexer_ByteLoader understands 
 * the position independently of its meaning. The LexatomLoader already 
 * interprets the stream as 'lexatoms'. A Calc_lexer_ByteLoader sets the stream 
...
#include "lexical_analysis/lib/quex/MemoryManager"
 ...
 void  Calc_lexer_LexatomLoader_lexatom_index_reset_backup(Calc_lexer_LexatomLoader* me, 
                                             Calc_lexer_stream_position_t Backup_lexatom_index_next_to_fill, 
                                             ptrdiff_t                 BackupStomachByteN, 
                                             Calc_lexer_stream_position_t BackupByteLoaderPosition);
...
Calc_lexer_LexatomLoader_lexatom_index_tell(Calc_lexer_LexatomLoader* me)
{
 ...
 bool 
Calc_lexer_LexatomLoader_lexatom_index_seek(Calc_lexer_LexatomLoader*         me, 
                                             const Calc_lexer_stream_position_t  CharacterIndex) 
...
{ 
    ...
    Calc_lexer_stream_position_t      backup_byte_loader_position; 
    Calc_lexer_stream_position_t      backup_lexatom_index_next_to_fill;
    Calc_lexer_stream_position_t      target_byte_pos;
    ...
    if( CharacterIndex < me->lexatom_index_next_to_fill ) {
        ...
        if( ! Calc_lexer_LexatomLoader_lexatom_index_reset(me) ) {
            return false;
        }
    }
    ...
    if( ! Calc_lexer_LexatomLoader_lexatom_index_step_to(me, (ptrdiff_t)CharacterIndex) ) {
        Calc_lexer_LexatomLoader_lexatom_index_reset_backup(me, 
 ...
 *          false, if positioning failed.                                    */
{
 ...
 void
Calc_lexer_LexatomLoader_lexatom_index_reset_backup(Calc_lexer_LexatomLoader* me, 
                                                    Calc_lexer_stream_position_t Backup_lexatom_index_next_to_fill, 
                                                    ptrdiff_t                 BackupStomachByteN, 
                                                    Calc_lexer_stream_position_t BackupByteLoaderPosition)
/* Reset a previous state of the LexatomLoader and its Calc_lexer_ByteLoader.            */
{
    Calc_lexer_stream_position_t backup_byte_pos;
    ...
    if( BackupStomachByteN == -1 ) {
        ...
        else if( ! Calc_lexer_LexatomLoader_lexatom_index_step_to(me, (ptrdiff_t)Backup_lexatom_index_next_to_fill) ) {
 ...
 bool 
Calc_lexer_LexatomLoader_lexatom_index_step_to(Calc_lexer_LexatomLoader*       me,
...
{ 
    const Calc_lexer_stream_position_t ChunkSize = QUEX_Calc_lexer_SETTING_BUFFER_LEXATOM_LOADER_SEEK_BUFFER_SIZE;
    Calc_lexer_lexatom_t               chunk[QUEX_Calc_lexer_SETTING_BUFFER_LEXATOM_LOADER_SEEK_BUFFER_SIZE];
    Calc_lexer_stream_position_t       remaining_n      = TargetCI - me->lexatom_index_next_to_fill;
    bool                            end_of_stream_f  = false;
    bool                            encoding_error_f = false;
    size_t                          loaded_n;

    __quex_assert(QUEX_Calc_lexer_SETTING_BUFFER_LEXATOM_LOADER_SEEK_BUFFER_SIZE >= 1);
    ...
    for(; remaining_n > ChunkSize; remaining_n -= ChunkSize ) {
        ...
        me->lexatom_index_next_to_fill += (Calc_lexer_stream_position_t)loaded_n;
        if( ChunkSize > (Calc_lexer_stream_position_t)loaded_n ) {
            return false;
        }
    }
    if( remaining_n ) {
        loaded_n = me->derived.load_lexatoms(me, &chunk[0], (size_t)remaining_n, 
                                             &end_of_stream_f, &encoding_error_f);
        me->lexatom_index_next_to_fill += (Calc_lexer_stream_position_t)loaded_n;
        if( remaining_n > (Calc_lexer_stream_position_t)loaded_n ) {
            return false;
        }
    }
...
#endif /*   QUEX_INCLUDE_GUARD_Calc_lexer__BUFFER__LEXATOMS__LEXATOM_LOADER_NAVIGATION_I */
