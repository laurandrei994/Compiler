...
#ifndef  QUEX_INCLUDE_GUARD_quexProcessed__ANALYZER__STRUCT__INCLUDE_STACK_I
#define  QUEX_INCLUDE_GUARD_quexProcessed__ANALYZER__STRUCT__INCLUDE_STACK_I




inline bool
quexProcessed_Memento_construct(quexProcessed_Memento* memento,
                             quexProcessed* me,
                             const char*         InputNameP);

inline bool
quexProcessed_MF_include_push_file_name(quexProcessed*     me,
                                  const char*             FileName, 
                                  quexProcessed_Converter*   new_converter /* = 0 */)
{
    ...
    else if( ! quexProcessed_MF_include_push_ByteLoader(me, FileName, new_byte_loader, new_converter) ) {
        goto ERROR_1;
    }
    return true;
...
inline bool
quexProcessed_MF_include_push_ByteLoader(quexProcessed*     me,
                                      ...
                                      quexProcessed_Converter*   new_converter /* = 0 */)
{
    ...
    if( ! quexProcessed_MF_include_push_core(me, InputName, (quexProcessed_Buffer*)0,
                                          new_filler) ) {
        goto ERROR_1;
    }
    return true;
...
inline bool
quexProcessed_MF_include_push_memory(quexProcessed* me,
                                  const char*         InputName,
 ...
 * loader', and 'no buffer filler'.                                           */
{
    ...
    if( quexProcessed_Buffer_has_intersection(&me->buffer, Memory, (ptrdiff_t)MemorySize) ) {
        quexProcessed_MF_error_code_set_if_first(me, E_Error_ProvidedExternal_Memory_Intersects);
        goto ERROR_0;
    }
    else if( ! quexProcessed_BufferMemory_check_chunk(Memory, MemorySize, EndOfFileP) ) {
        quexProcessed_MF_error_code_set_if_first(me, E_Error_ProvidedExternal_Memory_Corrupt);
        goto ERROR_0;
    }
    else if( me->error_code != E_Error_None ) {
        quexProcessed_MF_error_code_set_if_first(me, E_Error_IncludePush_OnError);
        goto ERROR_0;
    }

    /* The 'new_buffer' is only copied including the reference to the new 
     * memory. However, the box object 'new_buffer' is left alone.            */
    quexProcessed_Buffer_construct(&new_buffer, 
    ...
    if( ! quexProcessed_MF_include_push_core(me, InputName, &new_buffer,
                                          (quexProcessed_LexatomLoader*)0) ) {
        goto ERROR_1;
    }
    return true;
    ...
    /* ERROR CASES: Free Resources ___________________________________________*/
ERROR_1:
    quexProcessed_Buffer_destruct(&new_buffer);
ERROR_0:
    return false;
}

inline bool
quexProcessed_MF_include_push_core(quexProcessed*       me,
                                const char*               InputNameP,
                                quexProcessed_Buffer*        new_buffer,
                                quexProcessed_LexatomLoader* new_filler)
{
    ...
    memento = (quexProcessed_Memento*)quex::MemoryManager_allocate(
                      sizeof(quexProcessed_Memento), E_MemoryObjectType_MEMENTO);
    if( ! memento ) {
        goto ERROR_1;
    }
    else if( ! quexProcessed_Memento_construct(memento, me, InputNameP) ) {
        goto ERROR_2;
    }
    else if( ! new_buffer ) {
        /* Buffer pointers are already copied into 'memento->buffer'.
         * Construct the new buffer directly into the current buffer.         */
        if( ! quexProcessed_Buffer_nested_construct(&me->buffer, &memento->buffer, 
                                                 new_filler) ) {
            goto ERROR_3;
        }
    }
    else {
        quexProcessed_Buffer_shallow_copy(&me->buffer, new_buffer);
    }

    __quex_assert(me->buffer.begin(&me->buffer) != memento->buffer.begin(&memento->buffer));
    quexProcessed_Buffer_assert_consistency(&memento->buffer);
    quexProcessed_Buffer_assert_consistency(&me->buffer);

     (void)quexProcessed_Counter_construct(&me->counter);
...
ERROR_3:
    quexProcessed_Buffer_shallow_copy(&me->buffer, &memento->buffer);
ERROR_2:
...
#   endif
    ...
    if( ! quexProcessed_user_memento_pack(me, InputNameP, memento) ) {
        quexProcessed_MF_error_code_set_if_first(me, E_Error_UserMementoPack_Failed);
        return false;
    }
    return true;
}   
...
inline bool
quexProcessed_MF_include_pop(quexProcessed* me)
 ...
 *          false, else.                                                     */
{
    ...
    if( ! me->_parent_memento ) {
        quexProcessed_MF_error_code_set_if_first(me, E_Error_IncludePopOnEmptyStack);
        return false;                             
    }
...
inline void
quexProcessed_MF_include_stack_delete(quexProcessed* me)
{
    /* Avoid 'E_Error_IncludePopOnEmptyStack'. Check before.                 */
    if( ! me->_parent_memento ) return;
    while( quexProcessed_MF_include_pop(me) );
}

inline bool
quexProcessed_MF_include_detect_recursion(quexProcessed* me,
                                       const char*         InputName)
{
    quexProcessed_Memento* iterator;
...
#endif /*  QUEX_INCLUDE_GUARD_quexProcessed__ANALYZER__STRUCT__INCLUDE_STACK_I */
