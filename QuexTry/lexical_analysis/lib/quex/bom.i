...
#include "lexical_analysis/lib/quex/bom"
 ...
 * this is not the case then use the **binary** option of your stream.       */
{
    ...
    result = quex_bom_snap_core(buffer, read_n, &byte_n);
    ...
    p0 += (Calc_lexer_stream_position_t)byte_n;
...
extern E_ByteOrderMark
quex_bom_snap_core(uint8_t buffer[4], size_t read_n, size_t* byte_n)
...
extern const char*
quex_bom_name(E_ByteOrderMark BOM)
{
    switch( BOM ) {
