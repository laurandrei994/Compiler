...
#ifndef QUEX_INCLUDE_GUARD_compilator__ANALYZER__STRUCT__RESET_I
#define QUEX_INCLUDE_GUARD_compilator__ANALYZER__STRUCT__RESET_I

#include "compilator\lib\buffer/Buffer.i"
#include "compilator\lib\analyzer/struct/reset"



inline bool
compilator_MF_reset(compilator* me)  
...
{
    if( me->buffer.filler ) {
        if( ! compilator_LexatomLoader_lexatom_index_reset(me->buffer.filler) ) {
            goto ERROR_0;
        }
    }

    compilator_Buffer_init(&me->buffer, (compilator_lexatom_t*)0); 

    if( ! compilator_MF_reset_all_but_buffer(me) ) {
        goto ERROR_0;
    }
...
inline bool
compilator_MF_reset_file_name(compilator*   me, 
                           const char*           FileName, 
 ...
 *          false, in case of failure.                                        */
{
    ...
    if( ! compilator_MF_reset_ByteLoader(me, new_byte_loader, new_converter) ) {
        goto ERROR_1;
    }
...
inline bool
compilator_MF_reset_ByteLoader(compilator*     me,
                            compilator_ByteLoader*  new_byte_loader,
                            compilator_Converter*   new_converter /* = 0 */)
 ...
 *          false, in case of failure.                                        */
{
    ...
    if( me->error_code != E_Error_None ) {
        compilator_MF_error_code_set_if_first(me, E_Error_Reset_OnError);
        goto ERROR_0;
    }
...
ERROR_0:
    ...
    /* 'destruct' marks resources as absent => double destruction is safe.    */
ERROR_1:
    compilator_destruct(me);  /* Destructs also 'me->buffer'                  */
    return false;
}

inline bool
compilator_MF_reset_memory(compilator*  me, 
...
inline bool
compilator_MF_reset_all_but_buffer(compilator*  me) 
 ...
 *          false, else. Buffer's resource must be destructed/marked absent.  */
{
    ...
    if( ! compilator_user_reset(me) ) {
        compilator_MF_error_code_set_if_first(me, E_Error_UserReset_Failed);
        goto ERROR_0;
    }
    else if( ! compilator_construct_all_but_buffer(me, false) ) {
        goto ERROR_0;
    }
    return true;
...
#endif /* QUEX_INCLUDE_GUARD_compilator__ANALYZER__STRUCT__RESET_I */
