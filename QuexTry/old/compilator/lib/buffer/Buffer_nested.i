/* vim:set ft=c: -*- C++ -*- */
#ifndef QUEX_INCLUDE_GUARD_compilator__BUFFER__BUFFER_NESTED_I
#define QUEX_INCLUDE_GUARD_compilator__BUFFER__BUFFER_NESTED_I
...
inline void
compilator_Buffer_adapt_to_new_memory_location_root(compilator_Buffer* me,
                                                    compilator_lexatom_t* old_memory_root,
                                                    compilator_lexatom_t* new_memory_root,
                                                    ptrdiff_t          NewRootSize);

inline bool
compilator_Buffer_nested_construct(compilator_Buffer*        me,
                                   compilator_Buffer*        nesting,
 ...
 *                                                                            */
{
    ...
    compilator_Buffer*  nesting_buffer_p = (compilator_Buffer*)0;


    compilator_Buffer_assert_consistency(nesting);

    if( compilator_Buffer_resources_absent(nesting) ) {
        goto ERROR_0;
    }
    else if( available_size < (ptrdiff_t)(QUEX_compilator_SETTING_BUFFER_SIZE_MIN) ) {
        /* (1) AVAILABLE SIZE too SMALL
         *     => Try to move content, so that free space becomes available.  */                    
        available_size = compilator_Buffer_nested_free_front(nesting);
    }

    if( available_size < (ptrdiff_t)QUEX_compilator_SETTING_BUFFER_SIZE_MIN ) {
        /* (2) AVAILABLE SIZE still too SMALL
         *     => Allocate new memory for new buffer.                         */                    
        memory_size = (size_t)(QUEX_compilator_SETTING_BUFFER_SIZE);
        memory      = (compilator_lexatom_t*)quex::MemoryManager_allocate(
                                memory_size * sizeof(compilator_lexatom_t), 
                                E_MemoryObjectType_BUFFER_MEMORY);
        if( ! memory ) {
            goto ERROR_0;
        }
        ownership        = E_Ownership_LEXICAL_ANALYZER;
        nesting_buffer_p = (compilator_Buffer*)0;
    }
    else {
    ...
    compilator_Buffer_construct(me, filler, memory, memory_size, 
                                (compilator_lexatom_t*)0, me->_fallback_n,
...
inline bool
compilator_Buffer_nested_negotiate_extend(compilator_Buffer*  me, 
                                          float               Factor)
 ...
 *          false, else.                                                      */
{
    ...
    while( ! compilator_Buffer_nested_extend(me, new_size - current_size) ) {
...
inline bool
compilator_Buffer_nested_extend(compilator_Buffer*  me, ptrdiff_t  SizeAdd)
 ...
 *          false, else.                                                      */
{
    compilator_lexatom_t*  new_memory_root_p;
    ptrdiff_t           required_size;
    ptrdiff_t           new_size;
    compilator_Buffer*  root = me;
    compilator_Buffer*  focus = me;
    ...
    root              = compilator_Buffer_nested_find_root(me);
    compilator_Buffer_assert_consistency(root);
    ...
    for(focus = me; focus ; focus = focus->_memory.including_buffer) {
        compilator_Buffer_callbacks_on_buffer_before_change(focus);
    }

    new_memory_root_p = (compilator_lexatom_t*)quex::MemoryManager_reallocate(
                                                (void*)root->begin(root),
                                                sizeof(compilator_lexatom_t) * (size_t)new_size,
                                                E_MemoryObjectType_BUFFER_MEMORY);
    ...
    if( ! new_memory_root_p ) {
        ...
        me->_memory._back[0] = QUEX_compilator_SETTING_BUFFER_LEXATOM_BUFFER_BORDER;
        verdict_f = true;
    }
    else {
        compilator_Buffer_adapt_to_new_memory_location_root(me, 
                                                            root->begin(root),
                                                            new_memory_root_p, 
                                                            new_size);
...
inline bool
compilator_Buffer_nested_migrate(compilator_Buffer*  me,
                                 compilator_lexatom_t*  memory,
...
{
    compilator_Buffer* root;
    compilator_Buffer* focus;
    compilator_lexatom_t* old_memory_root_p;
    compilator_lexatom_t* old_content_end_p = me->content_end(me);
    ptrdiff_t          required_size;
    bool               verdict_f = false;
    ...
    if( required_size > MemoryLexatomN ) {
        ...
        __QUEX_STD_memcpy((void*)&memory[0], 
                          ...
                          (size_t)required_size * sizeof(compilator_lexatom_t));
        ...
        compilator_Buffer_adapt_to_new_memory_location_root(me, old_memory_root_p,
...
inline compilator_Buffer*
compilator_Buffer_nested_find_root(compilator_Buffer* me)
...
inline ptrdiff_t
compilator_Buffer_nested_free_front(compilator_Buffer* me)
/* Shrink all nesting buffers to a minimum and reset all pointers accordingly.
 *
 * RETURNS: Free space at the end of 'me'.                                    */
{
    compilator_Buffer* it;
    compilator_Buffer* previous = (compilator_Buffer*)0;
    ...
    for(it = compilator_Buffer_nested_find_root(me); true ; 
        it = compilator_Buffer_nested_get(it, me)) {
        move_distance = compilator_Buffer_move_get_max_distance_towards_begin(it);

        if( previous ) {
            previous->_memory._back   -= nesting_freed;
            it->_memory._front        -= nesting_freed;
            previous->_memory._back[0] = (compilator_lexatom_t)(QUEX_compilator_SETTING_BUFFER_LEXATOM_BUFFER_BORDER);
            it->_memory._front[0]      = (compilator_lexatom_t)(QUEX_compilator_SETTING_BUFFER_LEXATOM_BUFFER_BORDER);
        }

        compilator_Buffer_move_towards_begin(it, move_distance + nesting_freed,
                                             (compilator_lexatom_t**)0, 0);
...
{
    compilator_Buffer* focus = (compilator_Buffer*)0;
    compilator_lexatom_t* new_memory;
    ptrdiff_t          buffer_size;
    /* NOT: compilator_Buffer_assert_consistency(me)
    ...
    for(focus = me; focus ; focus = focus->_memory.including_buffer) {
        ...
        compilator_Buffer_adapt_to_new_memory_location(focus, new_memory, buffer_size);
    }
...
inline bool
compilator_Buffer_adapt_to_new_memory_location(compilator_Buffer* me,
                                               compilator_lexatom_t* new_memory_base,
                                               ptrdiff_t          NewSize)
 ...
 *                                                                            */
{
    ...
    compilator_BufferMemory_construct(&me->_memory, new_memory_base, (size_t)NewSize,
                                      me->_memory.ownership, 
                                      me->_memory.including_buffer); 

    compilator_Buffer_init_content_core(me, 
    ...
    /*                                */  (compilator_lexatom_t)0, /* ignored */
...
inline compilator_Buffer* 
compilator_Buffer_nested_get(compilator_Buffer* me, 
...
#endif /* QUEX_INCLUDE_GUARD_compilator__BUFFER__BUFFER_NESTED_I */
