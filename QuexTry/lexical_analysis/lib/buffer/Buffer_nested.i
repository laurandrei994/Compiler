/* vim:set ft=c: -*- C++ -*- */
#ifndef QUEX_INCLUDE_GUARD_Calc_lexer__BUFFER__BUFFER_NESTED_I
#define QUEX_INCLUDE_GUARD_Calc_lexer__BUFFER__BUFFER_NESTED_I
...
#include "lexical_analysis/lib/quex/MemoryManager"
 ...
 void
Calc_lexer_Buffer_adapt_to_new_memory_location_root(Calc_lexer_Buffer* me,
                                                    Calc_lexer_lexatom_t* old_memory_root,
                                                    Calc_lexer_lexatom_t* new_memory_root,
                                                    ptrdiff_t          NewRootSize);

 bool
Calc_lexer_Buffer_nested_construct(Calc_lexer_Buffer*        me,
                                   Calc_lexer_Buffer*        nesting,
 ...
 *                                                                            */
{
    ...
    Calc_lexer_Buffer*  nesting_buffer_p = (Calc_lexer_Buffer*)0;


    Calc_lexer_Buffer_assert_consistency(nesting);

    if( Calc_lexer_Buffer_resources_absent(nesting) ) {
        goto ERROR_0;
    }
    else if( available_size < (ptrdiff_t)(QUEX_Calc_lexer_SETTING_BUFFER_SIZE_MIN) ) {
        /* (1) AVAILABLE SIZE too SMALL
         *     => Try to move content, so that free space becomes available.  */                    
        available_size = Calc_lexer_Buffer_nested_free_front(nesting);
    }

    if( available_size < (ptrdiff_t)QUEX_Calc_lexer_SETTING_BUFFER_SIZE_MIN ) {
        /* (2) AVAILABLE SIZE still too SMALL
         *     => Allocate new memory for new buffer.                         */                    
        memory_size = (size_t)(QUEX_Calc_lexer_SETTING_BUFFER_SIZE);
        memory      = (Calc_lexer_lexatom_t*)quex_MemoryManager_allocate(
                                memory_size * sizeof(Calc_lexer_lexatom_t), 
                                E_MemoryObjectType_BUFFER_MEMORY);
        if( ! memory ) {
            goto ERROR_0;
        }
        ownership        = E_Ownership_LEXICAL_ANALYZER;
        nesting_buffer_p = (Calc_lexer_Buffer*)0;
    }
    else {
    ...
    Calc_lexer_Buffer_construct(me, filler, memory, memory_size, 
                                (Calc_lexer_lexatom_t*)0, me->_fallback_n,
...
ERROR_0:
 ...
 bool
Calc_lexer_Buffer_nested_negotiate_extend(Calc_lexer_Buffer*  me, 
                                          float               Factor)
 ...
 *          false, else.                                                      */
{
    ...
    while( ! Calc_lexer_Buffer_nested_extend(me, new_size - current_size) ) {
 ...
 bool
Calc_lexer_Buffer_nested_extend(Calc_lexer_Buffer*  me, ptrdiff_t  SizeAdd)
 ...
 *          false, else.                                                      */
{
    Calc_lexer_lexatom_t*  new_memory_root_p;
    ptrdiff_t           required_size;
    ptrdiff_t           new_size;
    Calc_lexer_Buffer*  root = me;
    Calc_lexer_Buffer*  focus = me;
    ...
    root              = Calc_lexer_Buffer_nested_find_root(me);
    Calc_lexer_Buffer_assert_consistency(root);
    ...
    for(focus = me; focus ; focus = focus->_memory.including_buffer) {
        Calc_lexer_Buffer_callbacks_on_buffer_before_change(focus);
    }

    new_memory_root_p = (Calc_lexer_lexatom_t*)quex_MemoryManager_reallocate(
                                                (void*)root->begin(root),
                                                sizeof(Calc_lexer_lexatom_t) * (size_t)new_size,
                                                E_MemoryObjectType_BUFFER_MEMORY);
    ...
    if( ! new_memory_root_p ) {
        ...
        me->_memory._back[0] = QUEX_Calc_lexer_SETTING_BUFFER_LEXATOM_BUFFER_BORDER;
        verdict_f = true;
    }
    else {
        Calc_lexer_Buffer_adapt_to_new_memory_location_root(me, 
                                                            root->begin(root),
                                                            new_memory_root_p, 
                                                            new_size);
 ...
 bool
Calc_lexer_Buffer_nested_migrate(Calc_lexer_Buffer*  me,
                                 Calc_lexer_lexatom_t*  memory,
...
{
    Calc_lexer_Buffer* root;
    Calc_lexer_Buffer* focus;
    Calc_lexer_lexatom_t* old_memory_root_p;
    Calc_lexer_lexatom_t* old_content_end_p = me->content_end(me);
    ptrdiff_t          required_size;
    bool               verdict_f = false;
    ...
    if( required_size > MemoryLexatomN ) {
        ...
        __QUEX_STD_memcpy((void*)&memory[0], 
                          ...
                          (size_t)required_size * sizeof(Calc_lexer_lexatom_t));
        ...
        Calc_lexer_Buffer_adapt_to_new_memory_location_root(me, old_memory_root_p,
                                                            &memory[0], MemoryLexatomN);

        if( root->_memory.ownership == E_Ownership_LEXICAL_ANALYZER ) {
            quex_MemoryManager_free(old_memory_root_p, E_MemoryObjectType_BUFFER_MEMORY);
        }
 ...
 Calc_lexer_Buffer*
Calc_lexer_Buffer_nested_find_root(Calc_lexer_Buffer* me)
 ...
 * RETURNS: Pointer to root buffer object.                                */
{
 ...
 ptrdiff_t
Calc_lexer_Buffer_nested_free_front(Calc_lexer_Buffer* me)
/* Shrink all nesting buffers to a minimum and reset all pointers accordingly.
 *
 * RETURNS: Free space at the end of 'me'.                                    */
{
    Calc_lexer_Buffer* it;
    Calc_lexer_Buffer* previous = (Calc_lexer_Buffer*)0;
    ...
    for(it = Calc_lexer_Buffer_nested_find_root(me); true ; 
        it = Calc_lexer_Buffer_nested_get(it, me)) {
        move_distance = Calc_lexer_Buffer_move_get_max_distance_towards_begin(it);

        if( previous ) {
            previous->_memory._back   -= nesting_freed;
            it->_memory._front        -= nesting_freed;
            previous->_memory._back[0] = (Calc_lexer_lexatom_t)(QUEX_Calc_lexer_SETTING_BUFFER_LEXATOM_BUFFER_BORDER);
            it->_memory._front[0]      = (Calc_lexer_lexatom_t)(QUEX_Calc_lexer_SETTING_BUFFER_LEXATOM_BUFFER_BORDER);
        }

        Calc_lexer_Buffer_move_towards_begin(it, move_distance + nesting_freed,
                                             (Calc_lexer_lexatom_t**)0, 0);
...
{
    Calc_lexer_Buffer* focus = (Calc_lexer_Buffer*)0;
    Calc_lexer_lexatom_t* new_memory;
    ptrdiff_t          buffer_size;
    /* NOT: Calc_lexer_Buffer_assert_consistency(me)
    ...
    for(focus = me; focus ; focus = focus->_memory.including_buffer) {
        ...
        Calc_lexer_Buffer_adapt_to_new_memory_location(focus, new_memory, buffer_size);
    }
 ...
 bool
Calc_lexer_Buffer_adapt_to_new_memory_location(Calc_lexer_Buffer* me,
                                               Calc_lexer_lexatom_t* new_memory_base,
                                               ptrdiff_t          NewSize)
 ...
 *                                                                            */
{
    ...
    Calc_lexer_BufferMemory_construct(&me->_memory, new_memory_base, (size_t)NewSize,
                                      me->_memory.ownership, 
                                      me->_memory.including_buffer); 

    Calc_lexer_Buffer_init_content_core(me, 
    ...
    /*                                */  (Calc_lexer_lexatom_t)0, /* ignored */
 ...
 Calc_lexer_Buffer* 
Calc_lexer_Buffer_nested_get(Calc_lexer_Buffer* me, 
...
#endif /* QUEX_INCLUDE_GUARD_Calc_lexer__BUFFER__BUFFER_NESTED_I */
