...
#ifndef QUEX_INCLUDE_GUARD_quexProcessed__BUFFER__BUFFER_NAVIGATION_I
#define QUEX_INCLUDE_GUARD_quexProcessed__BUFFER__BUFFER_NAVIGATION_I
...
inline bool  
quexProcessed_Buffer_seek_forward(quexProcessed_Buffer* me, const ptrdiff_t CharacterN)
 ...
 *          False -- else.                                                    */
{
    quexProcessed_stream_position_t  CharacterIndexAtReadP =   me->input.lexatom_index_begin
                                                       + (me->_read_p - me->content_begin(me));
    quexProcessed_stream_position_t  target = CharacterIndexAtReadP + CharacterN;

    if( ! CharacterN ) {
        return true;
    }
    else if( target < quexProcessed_Buffer_input_lexatom_index_end(me) ) {
        ...
        if( ! quexProcessed_Buffer_load_forward_to_contain(me, target) ) {
    ...
    return quexProcessed_Buffer_finish_seek_based_on_read_p(me);
}

inline bool  
quexProcessed_Buffer_seek_backward(quexProcessed_Buffer* me, 
 ...
 *          False -- else.                                                   */
{
    ...
    if( ! CharacterN ) {
        ...
        offset                  = (ptrdiff_t)QUEX_MIN((quexProcessed_stream_position_t)(ContentSpaceSize >> 1), target);
        new_lexatom_index_begin = target - offset;

        if( ! quexProcessed_Buffer_load_backward_to_contain(me, new_lexatom_index_begin) ) {
...
inline quexProcessed_stream_position_t  
quexProcessed_Buffer_tell(quexProcessed_Buffer* me)
/* RETURNS: lexatom index which corresponds to the position of the input
 *          pointer.                                                         */
{
    const quexProcessed_stream_position_t Delta = me->_read_p - me->content_begin(me);
    return Delta + me->input.lexatom_index_begin;
}

inline bool    
quexProcessed_Buffer_seek(quexProcessed_Buffer*              me, 
 ...
 *          totally fail. Then, check 'quexProcessed_Buffer_dysfunctional'.      */
{
    const quexProcessed_stream_position_t lexatom_index_read_p = quexProcessed_Buffer_tell(me);
    bool  verdict_f = false;

    if( LexatomIndex > lexatom_index_read_p ) {
        verdict_f = quexProcessed_Buffer_seek_forward(me, (ptrdiff_t)(LexatomIndex - lexatom_index_read_p));
    }
    else if( LexatomIndex < lexatom_index_read_p ) {
        verdict_f = quexProcessed_Buffer_seek_backward(me,(ptrdiff_t)(lexatom_index_read_p - LexatomIndex));
    }
    return verdict_f;
}

inline bool
quexProcessed_Buffer_finish_seek_based_on_read_p(quexProcessed_Buffer* me)
{
...
#endif                  /* QUEX_INCLUDE_GUARD_quexProcessed__BUFFER__BUFFER_NAVIGATION_I */
