...
#ifndef QUEX_INCLUDE_GUARD_Calc_lexer__BUFFER__BUFFER_NAVIGATION_I
#define QUEX_INCLUDE_GUARD_Calc_lexer__BUFFER__BUFFER_NAVIGATION_I
...
/* Determine lexatom index of first lexatom in the buffer.                    */
{
 ...
 bool  
Calc_lexer_Buffer_seek_forward(Calc_lexer_Buffer* me, const ptrdiff_t CharacterN)
 ...
 *          False -- else.                                                    */
{
    Calc_lexer_stream_position_t  CharacterIndexAtReadP =   me->input.lexatom_index_begin
                                                       + (me->_read_p - me->content_begin(me));
    Calc_lexer_stream_position_t  target = CharacterIndexAtReadP + CharacterN;

    if( ! CharacterN ) {
        return true;
    }
    else if( target < Calc_lexer_Buffer_input_lexatom_index_end(me) ) {
        ...
        if( ! Calc_lexer_Buffer_load_forward_to_contain(me, target) ) {
    ...
    return Calc_lexer_Buffer_finish_seek_based_on_read_p(me);
}

 bool  
Calc_lexer_Buffer_seek_backward(Calc_lexer_Buffer* me, 
 ...
 *          False -- else.                                                   */
{
    ...
    if( ! CharacterN ) {
        ...
        offset                  = (ptrdiff_t)QUEX_MIN((Calc_lexer_stream_position_t)(ContentSpaceSize >> 1), target);
        new_lexatom_index_begin = target - offset;

        if( ! Calc_lexer_Buffer_load_backward_to_contain(me, new_lexatom_index_begin) ) {
 ...
 Calc_lexer_stream_position_t  
Calc_lexer_Buffer_tell(Calc_lexer_Buffer* me)
/* RETURNS: lexatom index which corresponds to the position of the input
 *          pointer.                                                         */
{
    const Calc_lexer_stream_position_t Delta = me->_read_p - me->content_begin(me);
    return Delta + me->input.lexatom_index_begin;
}

 bool    
Calc_lexer_Buffer_seek(Calc_lexer_Buffer*              me, 
 ...
 *          totally fail. Then, check 'Calc_lexer_Buffer_dysfunctional'.      */
{
    const Calc_lexer_stream_position_t lexatom_index_read_p = Calc_lexer_Buffer_tell(me);
    bool  verdict_f = false;

    if( LexatomIndex > lexatom_index_read_p ) {
        verdict_f = Calc_lexer_Buffer_seek_forward(me, (ptrdiff_t)(LexatomIndex - lexatom_index_read_p));
    }
    else if( LexatomIndex < lexatom_index_read_p ) {
        verdict_f = Calc_lexer_Buffer_seek_backward(me,(ptrdiff_t)(lexatom_index_read_p - LexatomIndex));
    }
    return verdict_f;
}

 bool
Calc_lexer_Buffer_finish_seek_based_on_read_p(Calc_lexer_Buffer* me)
{
...
#endif                  /* QUEX_INCLUDE_GUARD_Calc_lexer__BUFFER__BUFFER_NAVIGATION_I */
