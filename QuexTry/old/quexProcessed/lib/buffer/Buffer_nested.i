/* vim:set ft=c: -*- C++ -*- */
#ifndef QUEX_INCLUDE_GUARD_quexProcessed__BUFFER__BUFFER_NESTED_I
#define QUEX_INCLUDE_GUARD_quexProcessed__BUFFER__BUFFER_NESTED_I
...
inline void
quexProcessed_Buffer_adapt_to_new_memory_location_root(quexProcessed_Buffer* me,
                                                    quexProcessed_lexatom_t* old_memory_root,
                                                    quexProcessed_lexatom_t* new_memory_root,
                                                    ptrdiff_t          NewRootSize);

inline bool
quexProcessed_Buffer_nested_construct(quexProcessed_Buffer*        me,
                                   quexProcessed_Buffer*        nesting,
 ...
 *                                                                            */
{
    ...
    quexProcessed_Buffer*  nesting_buffer_p = (quexProcessed_Buffer*)0;


    quexProcessed_Buffer_assert_consistency(nesting);

    if( quexProcessed_Buffer_resources_absent(nesting) ) {
        goto ERROR_0;
    }
    else if( available_size < (ptrdiff_t)(QUEX_quexProcessed_SETTING_BUFFER_SIZE_MIN) ) {
        /* (1) AVAILABLE SIZE too SMALL
         *     => Try to move content, so that free space becomes available.  */                    
        available_size = quexProcessed_Buffer_nested_free_front(nesting);
    }

    if( available_size < (ptrdiff_t)QUEX_quexProcessed_SETTING_BUFFER_SIZE_MIN ) {
        /* (2) AVAILABLE SIZE still too SMALL
         *     => Allocate new memory for new buffer.                         */                    
        memory_size = (size_t)(QUEX_quexProcessed_SETTING_BUFFER_SIZE);
        memory      = (quexProcessed_lexatom_t*)quex::MemoryManager_allocate(
                                memory_size * sizeof(quexProcessed_lexatom_t), 
                                E_MemoryObjectType_BUFFER_MEMORY);
        if( ! memory ) {
            goto ERROR_0;
        }
        ownership        = E_Ownership_LEXICAL_ANALYZER;
        nesting_buffer_p = (quexProcessed_Buffer*)0;
    }
    else {
    ...
    quexProcessed_Buffer_construct(me, filler, memory, memory_size, 
                                (quexProcessed_lexatom_t*)0, me->_fallback_n,
...
inline bool
quexProcessed_Buffer_nested_negotiate_extend(quexProcessed_Buffer*  me, 
                                          float               Factor)
 ...
 *          false, else.                                                      */
{
    ...
    while( ! quexProcessed_Buffer_nested_extend(me, new_size - current_size) ) {
...
inline bool
quexProcessed_Buffer_nested_extend(quexProcessed_Buffer*  me, ptrdiff_t  SizeAdd)
 ...
 *          false, else.                                                      */
{
    quexProcessed_lexatom_t*  new_memory_root_p;
    ptrdiff_t           required_size;
    ptrdiff_t           new_size;
    quexProcessed_Buffer*  root = me;
    quexProcessed_Buffer*  focus = me;
    ...
    root              = quexProcessed_Buffer_nested_find_root(me);
    quexProcessed_Buffer_assert_consistency(root);
    ...
    for(focus = me; focus ; focus = focus->_memory.including_buffer) {
        quexProcessed_Buffer_callbacks_on_buffer_before_change(focus);
    }

    new_memory_root_p = (quexProcessed_lexatom_t*)quex::MemoryManager_reallocate(
                                                (void*)root->begin(root),
                                                sizeof(quexProcessed_lexatom_t) * (size_t)new_size,
                                                E_MemoryObjectType_BUFFER_MEMORY);
    ...
    if( ! new_memory_root_p ) {
        ...
        me->_memory._back[0] = QUEX_quexProcessed_SETTING_BUFFER_LEXATOM_BUFFER_BORDER;
        verdict_f = true;
    }
    else {
        quexProcessed_Buffer_adapt_to_new_memory_location_root(me, 
                                                            root->begin(root),
                                                            new_memory_root_p, 
                                                            new_size);
...
inline bool
quexProcessed_Buffer_nested_migrate(quexProcessed_Buffer*  me,
                                 quexProcessed_lexatom_t*  memory,
...
{
    quexProcessed_Buffer* root;
    quexProcessed_Buffer* focus;
    quexProcessed_lexatom_t* old_memory_root_p;
    quexProcessed_lexatom_t* old_content_end_p = me->content_end(me);
    ptrdiff_t          required_size;
    bool               verdict_f = false;
    ...
    if( required_size > MemoryLexatomN ) {
        ...
        __QUEX_STD_memcpy((void*)&memory[0], 
                          ...
                          (size_t)required_size * sizeof(quexProcessed_lexatom_t));
        ...
        quexProcessed_Buffer_adapt_to_new_memory_location_root(me, old_memory_root_p,
...
inline quexProcessed_Buffer*
quexProcessed_Buffer_nested_find_root(quexProcessed_Buffer* me)
...
inline ptrdiff_t
quexProcessed_Buffer_nested_free_front(quexProcessed_Buffer* me)
/* Shrink all nesting buffers to a minimum and reset all pointers accordingly.
 *
 * RETURNS: Free space at the end of 'me'.                                    */
{
    quexProcessed_Buffer* it;
    quexProcessed_Buffer* previous = (quexProcessed_Buffer*)0;
    ...
    for(it = quexProcessed_Buffer_nested_find_root(me); true ; 
        it = quexProcessed_Buffer_nested_get(it, me)) {
        move_distance = quexProcessed_Buffer_move_get_max_distance_towards_begin(it);

        if( previous ) {
            previous->_memory._back   -= nesting_freed;
            it->_memory._front        -= nesting_freed;
            previous->_memory._back[0] = (quexProcessed_lexatom_t)(QUEX_quexProcessed_SETTING_BUFFER_LEXATOM_BUFFER_BORDER);
            it->_memory._front[0]      = (quexProcessed_lexatom_t)(QUEX_quexProcessed_SETTING_BUFFER_LEXATOM_BUFFER_BORDER);
        }

        quexProcessed_Buffer_move_towards_begin(it, move_distance + nesting_freed,
                                             (quexProcessed_lexatom_t**)0, 0);
...
{
    quexProcessed_Buffer* focus = (quexProcessed_Buffer*)0;
    quexProcessed_lexatom_t* new_memory;
    ptrdiff_t          buffer_size;
    /* NOT: quexProcessed_Buffer_assert_consistency(me)
    ...
    for(focus = me; focus ; focus = focus->_memory.including_buffer) {
        ...
        quexProcessed_Buffer_adapt_to_new_memory_location(focus, new_memory, buffer_size);
    }
...
inline bool
quexProcessed_Buffer_adapt_to_new_memory_location(quexProcessed_Buffer* me,
                                               quexProcessed_lexatom_t* new_memory_base,
                                               ptrdiff_t          NewSize)
 ...
 *                                                                            */
{
    ...
    quexProcessed_BufferMemory_construct(&me->_memory, new_memory_base, (size_t)NewSize,
                                      me->_memory.ownership, 
                                      me->_memory.including_buffer); 

    quexProcessed_Buffer_init_content_core(me, 
    ...
    /*                                */  (quexProcessed_lexatom_t)0, /* ignored */
...
inline quexProcessed_Buffer* 
quexProcessed_Buffer_nested_get(quexProcessed_Buffer* me, 
...
#endif /* QUEX_INCLUDE_GUARD_quexProcessed__BUFFER__BUFFER_NESTED_I */
