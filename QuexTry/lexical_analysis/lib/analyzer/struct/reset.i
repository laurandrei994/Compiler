...
#ifndef QUEX_INCLUDE_GUARD_Calc_lexer__ANALYZER__STRUCT__RESET_I
#define QUEX_INCLUDE_GUARD_Calc_lexer__ANALYZER__STRUCT__RESET_I

#include "lexical_analysis/lib/buffer/Buffer.i"
#include "lexical_analysis/lib/analyzer/struct/reset"



 bool
Calc_lexer_MF_reset(struct Calc_lexer_tag* me)  
...
{
    if( me->buffer.filler ) {
        if( ! Calc_lexer_LexatomLoader_lexatom_index_reset(me->buffer.filler) ) {
            goto ERROR_0;
        }
    }

    Calc_lexer_Buffer_init(&me->buffer, (Calc_lexer_lexatom_t*)0); 

    if( ! Calc_lexer_MF_reset_all_but_buffer(me) ) {
        goto ERROR_0;
    }
...
ERROR_0:
 ...
 bool
Calc_lexer_MF_reset_file_name(struct Calc_lexer_tag*   me, 
                           const char*           FileName, 
 ...
 *          false, in case of failure.                                        */
{
    ...
    if( ! Calc_lexer_MF_reset_ByteLoader(me, new_byte_loader, new_converter) ) {
        goto ERROR_1;
    }
 ...
 *      byte_loader = Calc_lexer_ByteLoader_stream_new(strangestr_p, false);  */
 bool
Calc_lexer_MF_reset_ByteLoader(struct Calc_lexer_tag*     me,
                            Calc_lexer_ByteLoader*  new_byte_loader,
                            Calc_lexer_Converter*   new_converter /* = 0 */)
 ...
 *          false, in case of failure.                                        */
{
    ...
    if( me->error_code != E_Error_None ) {
        Calc_lexer_MF_error_code_set_if_first(me, E_Error_Reset_OnError);
        goto ERROR_0;
    }
...
ERROR_0:
    ...
    /* 'destruct' marks resources as absent => double destruction is safe.    */
ERROR_1:
    Calc_lexer_destruct(me);  /* Destructs also 'me->buffer'                  */
    return false;
}

 bool
Calc_lexer_MF_reset_memory(struct Calc_lexer_tag*  me, 
...
ERROR_0:
 ...
 bool
Calc_lexer_MF_reset_all_but_buffer(struct Calc_lexer_tag*  me) 
 ...
 *          false, else. Buffer's resource must be destructed/marked absent.  */
{
    ...
    if( ! Calc_lexer_user_reset(me) ) {
        Calc_lexer_MF_error_code_set_if_first(me, E_Error_UserReset_Failed);
        goto ERROR_0;
    }
    else if( ! Calc_lexer_construct_all_but_buffer(me, false) ) {
        goto ERROR_0;
    }
    return true;
...
#endif /* QUEX_INCLUDE_GUARD_Calc_lexer__ANALYZER__STRUCT__RESET_I */
