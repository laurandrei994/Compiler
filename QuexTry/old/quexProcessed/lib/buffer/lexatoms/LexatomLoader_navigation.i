...
#ifndef  QUEX_INCLUDE_GUARD_quexProcessed__BUFFER__LEXATOMS__LEXATOM_LOADER_NAVIGATION_I
#define  QUEX_INCLUDE_GUARD_quexProcessed__BUFFER__LEXATOMS__LEXATOM_LOADER_NAVIGATION_I
 ...
 *    -- quexProcessed_ByteLoader's seek sets the position in the low level input
 *       stream.
 * 
 * A LexatomLoader's seek has NO direct influence on the content or the pointers
 * of the Buffer object that it fills.
 *
 * The difference between the quexProcessed_ByteLoader's seeking (stream seeking) 
 * and the LexatomLoader's seeking is that the quexProcessed_ByteLoader understands 
 * the position independently of its meaning. The LexatomLoader already 
 * interprets the stream as 'lexatoms'. A quexProcessed_ByteLoader sets the stream 
...
inline void  quexProcessed_LexatomLoader_lexatom_index_reset_backup(quexProcessed_LexatomLoader* me, 
                                             quexProcessed_stream_position_t Backup_lexatom_index_next_to_fill, 
                                             ptrdiff_t                 BackupStomachByteN, 
                                             quexProcessed_stream_position_t BackupByteLoaderPosition);
...
inline bool 
quexProcessed_LexatomLoader_lexatom_index_seek(quexProcessed_LexatomLoader*         me, 
                                             const quexProcessed_stream_position_t  CharacterIndex) 
...
{ 
    ...
    quexProcessed_stream_position_t      backup_byte_loader_position; 
    quexProcessed_stream_position_t      backup_lexatom_index_next_to_fill;
    quexProcessed_stream_position_t      target_byte_pos;
    ...
    if( CharacterIndex < me->lexatom_index_next_to_fill ) {
        ...
        if( ! quexProcessed_LexatomLoader_lexatom_index_reset(me) ) {
            return false;
        }
    }
    ...
    if( ! quexProcessed_LexatomLoader_lexatom_index_step_to(me, (ptrdiff_t)CharacterIndex) ) {
        quexProcessed_LexatomLoader_lexatom_index_reset_backup(me, 
...
inline void
quexProcessed_LexatomLoader_lexatom_index_reset_backup(quexProcessed_LexatomLoader* me, 
                                                    quexProcessed_stream_position_t Backup_lexatom_index_next_to_fill, 
                                                    ptrdiff_t                 BackupStomachByteN, 
                                                    quexProcessed_stream_position_t BackupByteLoaderPosition)
/* Reset a previous state of the LexatomLoader and its quexProcessed_ByteLoader.            */
{
    quexProcessed_stream_position_t backup_byte_pos;
    ...
    if( BackupStomachByteN == -1 ) {
        ...
        else if( ! quexProcessed_LexatomLoader_lexatom_index_step_to(me, (ptrdiff_t)Backup_lexatom_index_next_to_fill) ) {
...
inline bool 
quexProcessed_LexatomLoader_lexatom_index_step_to(quexProcessed_LexatomLoader*       me,
...
{ 
    const quexProcessed_stream_position_t ChunkSize = QUEX_quexProcessed_SETTING_BUFFER_LEXATOM_LOADER_SEEK_BUFFER_SIZE;
    quexProcessed_lexatom_t               chunk[QUEX_quexProcessed_SETTING_BUFFER_LEXATOM_LOADER_SEEK_BUFFER_SIZE];
    quexProcessed_stream_position_t       remaining_n      = TargetCI - me->lexatom_index_next_to_fill;
    bool                            end_of_stream_f  = false;
    bool                            encoding_error_f = false;
    size_t                          loaded_n;

    __quex_assert(QUEX_quexProcessed_SETTING_BUFFER_LEXATOM_LOADER_SEEK_BUFFER_SIZE >= 1);
    ...
    for(; remaining_n > ChunkSize; remaining_n -= ChunkSize ) {
        ...
        me->lexatom_index_next_to_fill += (quexProcessed_stream_position_t)loaded_n;
        if( ChunkSize > (quexProcessed_stream_position_t)loaded_n ) {
            return false;
        }
    }
    if( remaining_n ) {
        loaded_n = me->derived.load_lexatoms(me, &chunk[0], (size_t)remaining_n, 
                                             &end_of_stream_f, &encoding_error_f);
        me->lexatom_index_next_to_fill += (quexProcessed_stream_position_t)loaded_n;
        if( remaining_n > (quexProcessed_stream_position_t)loaded_n ) {
            return false;
        }
    }
...
#endif /*   QUEX_INCLUDE_GUARD_quexProcessed__BUFFER__LEXATOMS__LEXATOM_LOADER_NAVIGATION_I */
