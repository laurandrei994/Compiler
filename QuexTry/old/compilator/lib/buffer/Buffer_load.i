/* vim:set ft=c: -*- C++ -*- */
#ifndef QUEX_INCLUDE_GUARD_compilator__BUFFER__BUFFER_LOAD_I
#define QUEX_INCLUDE_GUARD_compilator__BUFFER__BUFFER_LOAD_I
...
inline E_LoadResult
compilator_Buffer_load_forward(compilator_Buffer*  me,
 ...
 *                                                                            */
{
    ...
    if( me->_read_p != me->content_end(me)) {
        ...
        compilator_Buffer_register_eos(me, me->input.lexatom_index_begin + me->content_size(me));
    ...
    if(    0 == move_distance 
        && ! compilator_Buffer_callbacks_on_cannot_move_towards_begin(me, &move_distance) ) {
        return E_LoadResult_OVERFLOW; 
    }

    if( ! compilator_Buffer_move_and_load(me, position_register, PositionRegisterN, 
...
inline bool
compilator_Buffer_load_forward_to_contain(compilator_Buffer*        me, 
 ...
 *           *disabled*. 
 *
 *        => In case of 'return false', call 'compilator_Buffer_dysfunctional */
{
    compilator_stream_position_t    lexatom_index_to_be_contained = LexatomIndexToBeContained;
    compilator_stream_position_t    lexatom_index_end = (compilator_stream_position_t)-1;
    ...
    move_distance = compilator_Buffer_move_get_distance_forward_to_contain(me, 
                                             &lexatom_index_to_be_contained);

    /* Even if 'move_distance = 0' call the 'move and load' for consistency.  */
    (void)compilator_Buffer_move_and_load(me, (compilator_lexatom_t**)0, 0,
    ...
    if( LexatomIndexToBeContained < lexatom_index_end ) {
        ...
        load_request_n = compilator_Buffer_move_towards_begin_undo(me, &bi, move_distance);

        loaded_n       = compilator_LexatomLoader_load(me->filler, me->content_begin(me), 
        ...
        if( loaded_n != load_request_n ) {
            ...
            compilator_Buffer_dysfunctional_set(me);
        }
        else {
...
inline E_LoadResult   
compilator_Buffer_load_backward(compilator_Buffer* me)
 ...
 *      to go farther back then the buffer's begin.                          */
{
    ...
    else if( ! compilator_ByteLoader_seek_is_enabled(me->filler->byte_loader) ) {
        return E_LoadResult_NO_MORE_DATA; /* Stream cannot go backwards.     */
    }

    move_distance = compilator_Buffer_move_get_max_distance_towards_end(me);
    ...
    loaded_n = compilator_Buffer_move_and_load_backward(me, move_distance, &encoding_error_f, &load_request_n);
...
inline bool
compilator_Buffer_load_backward_to_contain(compilator_Buffer*        me, 
                                           compilator_stream_position_t NewCharacterIndexBegin)
...
{
    /* compilator_lexatom_t* end_p    = me->content_space_end(me); */
    ptrdiff_t loaded_n;
    ptrdiff_t load_request_n;
    ptrdiff_t move_distance;
    bool      encoding_error_f = false;
    ...
    loaded_n = compilator_Buffer_move_and_load_backward(me, move_distance, 
                                                        &encoding_error_f, 
                                                        &load_request_n);
...
inline bool
compilator_Buffer_move_and_load(compilator_Buffer*  me, 
                                compilator_lexatom_t** position_register,
...
{
    compilator_stream_position_t   load_lexatom_index;
    ptrdiff_t                   free_space;
    bool                        end_of_stream_f  = false;

    compilator_Buffer_move_towards_begin(me, move_distance,
    ...
    *loaded_n = compilator_LexatomLoader_load(me->filler, me->content_end(me), 
...
inline ptrdiff_t
compilator_Buffer_move_and_load_backward(compilator_Buffer* me, 
 ...
 * RETURNS: Number of loaded lexatoms.                                        */
{
    ...
    (void)compilator_Buffer_move_towards_end(me, MoveDistance);

    *load_request_n = QUEX_MIN(me->content_space_end(me) - me->content_begin(me), 
                               MoveDistance);

    return compilator_LexatomLoader_load(me->filler, me->content_begin(me), *load_request_n,
...
#endif /* QUEX_INCLUDE_GUARD_compilator__BUFFER__BUFFER_LOAD_I */
