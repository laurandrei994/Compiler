/* vim:set ft=c: -*- C++ -*- */
#ifndef QUEX_INCLUDE_GUARD_compilator__BUFFER__BUFFER_MOVE_I
#define QUEX_INCLUDE_GUARD_compilator__BUFFER__BUFFER_MOVE_I
...
inline ptrdiff_t
compilator_Buffer_move_towards_begin(compilator_Buffer*  me, 
 ...
 * RETURNS: Number of free lexatoms from 'content_end' the end of buffer.     */
{
    ...
    if( MoveDistance ) {
        ...
        move_size = compilator_Buffer_move_size_towards_begin(me, MoveDistance);

        if( MoveDistance && move_size ) {
            __QUEX_STD_memmove((void*)me->content_begin(me), 
                               (void*)&me->content_begin(me)[MoveDistance],
                               (size_t)move_size * sizeof(compilator_lexatom_t));
        }

        compilator_Buffer_pointers_add_offset(me, - MoveDistance, 
...
inline ptrdiff_t
compilator_Buffer_move_towards_end(compilator_Buffer* me, 
                                   ptrdiff_t          MoveDistance)
...
{
    compilator_lexatom_t*  BeginP           = me->content_begin(me);
    ...
    if( MoveDistance ) {
        ...
        compilator_Buffer_move_backup_lexatom_index_of_lexeme_start(me, MoveDistance);
        ...
        compilator_Buffer_pointers_add_offset(me, MoveDistance, (compilator_lexatom_t**)0, 0);
    }
...
inline ptrdiff_t
compilator_Buffer_move_towards_begin_undo(compilator_Buffer*           me,
 ...
 * That is, they indicate the situation to be restored.                       */
{
    ...
    compilator_BufferInvariance_restore(bi, me);
    ...
    if( move_size && MoveDistance ) {
        /* NOT NECESSARY:
         *
         * compilator_Buffer_callbacks_on_buffer_before_change(me, BeginP);
         *
...
inline ptrdiff_t
compilator_Buffer_move_get_max_distance_towards_begin(compilator_Buffer*  me) 
 ...
 *        begin of the buffer.                                                */
{
    ...
    else if( compilator_Buffer_is_end_of_stream_inside(me) ) {
...
inline ptrdiff_t        
compilator_Buffer_move_get_max_distance_towards_end(compilator_Buffer* me)
...
inline ptrdiff_t
compilator_Buffer_move_size_towards_begin(compilator_Buffer* me, ptrdiff_t move_distance)
{
...
inline ptrdiff_t
compilator_Buffer_move_get_distance_forward_to_contain(compilator_Buffer*         me,
                                                       compilator_stream_position_t* lexatom_index_to_be_contained)
{
    compilator_stream_position_t lexatom_index_begin = me->input.lexatom_index_begin;
    compilator_stream_position_t lexatom_index_end   = lexatom_index_begin + me->content_size(me);
    compilator_stream_position_t new_lexatom_index_begin;
    const ptrdiff_t           FallBackN = me->_fallback_n;
    __quex_assert(FallBackN >= 0);
    ...
    __quex_assert(*lexatom_index_to_be_contained != (compilator_stream_position_t)-1);
    ...
    else if( me->input.lexatom_index_end_of_stream != (compilator_stream_position_t)-1 ) {
...
inline void
compilator_Buffer_move_backup_lexatom_index_of_lexeme_start(compilator_Buffer* me,
                                                            ptrdiff_t          MoveDistance)
 ...
 * can be targetted and 'normal' analysis can continue.                       */
{
    if(    me->_lexeme_start_p
        ...
        me->_backup_lexatom_index_of_lexeme_start_p =   compilator_Buffer_tell(me)
...
#endif /* QUEX_INCLUDE_GUARD_compilator__BUFFER__BUFFER_MOVE_I */
