...
#ifndef  QUEX_INCLUDE_GUARD_compilator__BUFFER__LEXATOMS__LEXATOM_LOADER_NAVIGATION_I
#define  QUEX_INCLUDE_GUARD_compilator__BUFFER__LEXATOMS__LEXATOM_LOADER_NAVIGATION_I
 ...
 *    -- compilator_ByteLoader's seek sets the position in the low level input
 *       stream.
 * 
 * A LexatomLoader's seek has NO direct influence on the content or the pointers
 * of the Buffer object that it fills.
 *
 * The difference between the compilator_ByteLoader's seeking (stream seeking) 
 * and the LexatomLoader's seeking is that the compilator_ByteLoader understands 
 * the position independently of its meaning. The LexatomLoader already 
 * interprets the stream as 'lexatoms'. A compilator_ByteLoader sets the stream 
...
inline void  compilator_LexatomLoader_lexatom_index_reset_backup(compilator_LexatomLoader* me, 
                                             compilator_stream_position_t Backup_lexatom_index_next_to_fill, 
                                             ptrdiff_t                 BackupStomachByteN, 
                                             compilator_stream_position_t BackupByteLoaderPosition);
...
inline bool 
compilator_LexatomLoader_lexatom_index_seek(compilator_LexatomLoader*         me, 
                                             const compilator_stream_position_t  CharacterIndex) 
...
{ 
    ...
    compilator_stream_position_t      backup_byte_loader_position; 
    compilator_stream_position_t      backup_lexatom_index_next_to_fill;
    compilator_stream_position_t      target_byte_pos;
    ...
    if( CharacterIndex < me->lexatom_index_next_to_fill ) {
        ...
        if( ! compilator_LexatomLoader_lexatom_index_reset(me) ) {
            return false;
        }
    }
    ...
    if( ! compilator_LexatomLoader_lexatom_index_step_to(me, (ptrdiff_t)CharacterIndex) ) {
        compilator_LexatomLoader_lexatom_index_reset_backup(me, 
...
inline void
compilator_LexatomLoader_lexatom_index_reset_backup(compilator_LexatomLoader* me, 
                                                    compilator_stream_position_t Backup_lexatom_index_next_to_fill, 
                                                    ptrdiff_t                 BackupStomachByteN, 
                                                    compilator_stream_position_t BackupByteLoaderPosition)
/* Reset a previous state of the LexatomLoader and its compilator_ByteLoader.            */
{
    compilator_stream_position_t backup_byte_pos;
    ...
    if( BackupStomachByteN == -1 ) {
        ...
        else if( ! compilator_LexatomLoader_lexatom_index_step_to(me, (ptrdiff_t)Backup_lexatom_index_next_to_fill) ) {
...
inline bool 
compilator_LexatomLoader_lexatom_index_step_to(compilator_LexatomLoader*       me,
...
{ 
    const compilator_stream_position_t ChunkSize = QUEX_compilator_SETTING_BUFFER_LEXATOM_LOADER_SEEK_BUFFER_SIZE;
    compilator_lexatom_t               chunk[QUEX_compilator_SETTING_BUFFER_LEXATOM_LOADER_SEEK_BUFFER_SIZE];
    compilator_stream_position_t       remaining_n      = TargetCI - me->lexatom_index_next_to_fill;
    bool                            end_of_stream_f  = false;
    bool                            encoding_error_f = false;
    size_t                          loaded_n;

    __quex_assert(QUEX_compilator_SETTING_BUFFER_LEXATOM_LOADER_SEEK_BUFFER_SIZE >= 1);
    ...
    for(; remaining_n > ChunkSize; remaining_n -= ChunkSize ) {
        ...
        me->lexatom_index_next_to_fill += (compilator_stream_position_t)loaded_n;
        if( ChunkSize > (compilator_stream_position_t)loaded_n ) {
            return false;
        }
    }
    if( remaining_n ) {
        loaded_n = me->derived.load_lexatoms(me, &chunk[0], (size_t)remaining_n, 
                                             &end_of_stream_f, &encoding_error_f);
        me->lexatom_index_next_to_fill += (compilator_stream_position_t)loaded_n;
        if( remaining_n > (compilator_stream_position_t)loaded_n ) {
            return false;
        }
    }
...
#endif /*   QUEX_INCLUDE_GUARD_compilator__BUFFER__LEXATOMS__LEXATOM_LOADER_NAVIGATION_I */
